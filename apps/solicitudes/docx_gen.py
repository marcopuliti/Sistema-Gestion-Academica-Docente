...
def generar_docx_solicitud_completa(solicitud):
    ...
    t_firma_n = doc.add_table(rows=1, cols=1)
    t_firma_n.style = 'Table Grid'
    cell_fn = t_firma_n.cell(0, 0)
    cell_fn.text = ''
    tc = cell_fn._tc
    tcPr = tc.get_or_add_tcPr()
    tcBorders = OxmlElement('w:tcBorders')
    ...
    cell_left.text = ''
    if os.path.exists(logo_path):
        p_logo2 = cell_left.paragraphs[0]
        p_logo2.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p_logo2.add_run().add_picture(logo_path, width=Cm(2.8))
